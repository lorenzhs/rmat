...
struct alias_key {
    ...
    static_assert((sizeof(double) / sizeof(key_type)) * sizeof(key_type) == sizeof(double),
                  "key_type has weird size");
    ...
    void construct(Iterator begin, Iterator end, bool is_dist = false) {
        ...
        if (is_dist) {
            ...
            W_ = std::accumulate(
                begin, end, 0.0,
                [](double curr, auto b) { return curr + b.second; });
        ...
        while (small != s_end && large != l_end) {
            LOG << "table_[" << table_idx << "] = (" << small->second
            ...
            if (is_small(large)) {
                sLOG << "large item became small, remaining piece"
    ...
    key_type sample(double uniform) const {
        ...
        assert(item.a != static_cast<key_type>(-1) ||
